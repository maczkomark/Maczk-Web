...
export const BackgroundBeams = React.memo(({ className }: { className?: string }) => {
  ...
  return (
    <div className={cn("absolute inset-0 flex h-full w-full items-center justify-center", className)}>
      <svg className="z-0 h-full w-full pointer-events-none absolute" viewBox="0 0 696 316" fill="none">
        <path
        ...
        />
        {paths.map((d, i) => (
          <motion.path key={`p-${i}`} d={d} stroke={`url(#lg-${i})`} strokeOpacity="0.4" strokeWidth="0.5" />
        ))}
        <defs>
          {paths.map((_, i) => (
